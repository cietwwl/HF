...
public class _503_PursuitClanAmbition extends Quest implements ScriptFile
{
	...
	public void setLeaderVar(QuestState st, String var, String value)
	{
		...
		// для прямой записи в базу cond не корректируем, при загрузке
		// сконвертится само.
		{
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(id == CREATED && npcId == Gustaf)
		{
			if(st.getPlayer().getClan() != null) // has Clan
			{
				if(isLeader) // check if player is clan leader
				{
					...
					if(st.getQuestItemsCount(Proof_Aspiration) > 0) // if he
					// has
					// the
					// proof
					// already, tell him
					...
					else if(clanLevel > 3) // if clanLevel > 3 you can take
						// this quest,
						// because repeatable
		...
		else if(st.getPlayer().getClan() != null && st.getPlayer().getClan().getLevel() == 5) // player has level
			// 5 clan
			// already
			return "completed";
		else
