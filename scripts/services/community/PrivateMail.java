...
import l2f.gameserver.model.entity.auction.*;
...
public class PrivateMail implements ScriptFile, ICommunityBoardHandler
{
	...
	public void onLoad()
	{
		...
		{
			_log.info("CommunityBoard: Mail service loaded.");
			...
			String itemGrade, String search, int itemSort, int gradeSort, int quantitySort, int priceSort)
	{
		...
		for (int i = 10*(page-1);i<Math.min(auctions.size(), 10*(page));i++)
		{
			...
			builder.append("<td width=32><button value=\"\" action=\"bypass _bbsAuction_ %page% _ %type% _ %grade% _ %search% _ %itemSort% _ %gradeSort% _ %quantitySort% _ %priceSort% _ 0 _ ").append(auction.getAuctionId()).append("\" width=32 height=32 back=\"icon.").append(item.getTemplate().getIcon()).append("\" fore=\"icon.").append(item.getTemplate().getIcon()).append("\"></td>");
			builder.append(getItemName(item, 248, 25));
			...
			String itemGrade, String search, int itemSort, int gradeSort, int quantitySort, int priceSort, int auctionId)
	{
		...
		html = html.replace("%fullName%", 	"<table width=240 height=50><tr>"+getItemName(choosenItem, 240, 50, (auction.getCountToSell() > 1 ? " x"+auction.getCountToSell() : ""))+"</tr></table>");
		...
		html = html.replace("%pricePerItem%", 	"<font color=94775b>"+String.valueOf(Util.getNumberWithCommas(auction
				.getPricePerItem()))+"</font>");
		html = html.replace("%totalPrice%", 	"<font color=94775b>"+String.valueOf(Util.getNumberWithCommas(auction
				.getCountToSell() * auction.getPricePerItem()))+"</font>");
	...
	private String fillNewAuctionPage(Player player, String html, boolean newItem, int currentItem, int line)
	{
		...
		for (int i = 6*(line);i<6*(line+3);i++)
		{
			...
			if (item != null)
				builder.append("<button value=\"\" action=\"bypass _bbsNewAuction_ n").append(item.getObjectId()).append(" _ ").append(line).append("\" width=32 height=32 back=\"icon.").append(item.getTemplate().getIcon()).append("\" fore=\"icon.").append(item.getTemplate().getIcon()).append("\">");
			else
				builder.append("<br>");
			builder.append("</td>");
		...
		for (;i<10;i++)
		{
			...
			builder.append("<button value=\"\" action=\"bypass _bbsNewAuction_ c").append(item.getObjectId()).append(" _ ").append(line).append(" _ 1\" width=32 height=32 back=\"icon.").append(item.getTemplate().getIcon()).append("\" fore=\"icon.").append(item.getTemplate().getIcon()).append("\">");
			builder.append("</td>");
			//getting item name
			builder.append(getItemName(item, 228, 25));
		...
		html = html.replace("%choosenItem%", (choosenItem != null ? (getItemName(choosenItem, 180, 45, (choosenItem.getCount() > 1 ? " x"+choosenItem.getCount() : ""))) : ""));
	 ...
	 */
	private String getItemName(ItemInstance item, int windowWidth, int windowHeight, String... addToItemName)
	{
		StringBuilder builder = new StringBuilder();
