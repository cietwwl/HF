...
public class CommunityClan extends Functions implements ScriptFile, ICommunityBoardHandler
{
	...
	private String parseNavigate(int page)
	{
		...
		double size = ClanRequest.getClanList().size();
		double inpage = 10;
		...
		if(size > inpage)
		{
			...
			for(int current = 1; current <= max; current++)
			{
				if(page == current)
					...
					pg.append("<td width=25 align=center><button value=\"").append(current).append("\" action=\"bypass _bbsclan:list:clan:").append(current).append("\" width=28 height=25 back=\"L2UI_ct1.button_df_down\" fore=\"L2UI_ct1.button_df\"></td>");
