	...
	public void onBypassCommand(Player activeChar, String command)
	{
		if(command.equals("_bbsechant"))
		{
			...
			for(int i = 0; i < len; i++)
			{
				...
				sb.append(new StringBuilder("<font color=\"LEVEL\">" + _item.getTemplate().getName() + " " + (_item.getEnchantLevel() <= 0 ? "" : new StringBuilder("</font><br1><font color=3293F3>Refined to: +" + _item.getEnchantLevel())) + "</font><br1>"));

				sb.append(new StringBuilder("Require item: <font color=\"LEVEL\">" + name + "</font>"));
				sb.append("<img src=\"l2ui.squaregray\" width=\"170\" height=\"1\">");
				sb.append("</td><td>");
				sb.append(new StringBuilder("<button value=\"Enchant\" action=\"bypass _bbsechant;enchlistpage;" + _item.getObjectId() + "\" width=75 height=18 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">"));
				sb.append("</td><td>");
				sb.append(new StringBuilder("<button value=\"Attribute\" action=\"bypass _bbsechant;enchlistpageAtrChus;" + _item.getObjectId() + "\" width=75 height=18 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">"));
				sb.append("</td></tr>");
			}
		...
		if(command.startsWith("_bbsechant;enchlistpage;"))
		{
			...
			StringBuilder sb = new StringBuilder();
			sb.append("For normal enchant selected item:<br1><table width=300>");
			...
			sb.append(new StringBuilder("<font color=\"LEVEL\">" + EhchantItem.getTemplate().getName() + " " + (EhchantItem.getEnchantLevel() <= 0 ? "" : new StringBuilder("</font><br1><font color=3293F3>Refined to: +" + EhchantItem.getEnchantLevel())) + "</font><br1>"));

			sb.append(new StringBuilder("Sharpening is a: <font color=\"LEVEL\">" + name + "</font>"));
		...
		if(command.startsWith("_bbsechant;enchlistpageAtrChus;"))
		{
			...
			StringBuilder sb = new StringBuilder();
			sb.append("Enchant the attribute selected item:<br1><table width=300>");
			sb.append(new StringBuilder("<tr><td width=32><img src=icon." + EhchantItem.getTemplate().getIcon() + " width=32 height=32> <img src=\"l2ui.squaregray\" width=\"32\" height=\"1\"></td><td width=236><center>"));
			sb.append(new StringBuilder("<font color=\"LEVEL\">" + EhchantItem.getTemplate().getName() + " " + (EhchantItem.getEnchantLevel() <= 0 ? "" : new StringBuilder("</font><br1><font color=3293F3>Refined to: +" + EhchantItem.getEnchantLevel())) + "</font><br1>"));

			sb.append(new StringBuilder("Enchant is a: <font color=\"LEVEL\">" + name + "</font>"));
			sb.append("<img src=\"l2ui.squaregray\" width=\"236\" height=\"1\"><center></td>");
			sb.append(new StringBuilder("<td width=32><img src=icon." + EhchantItem.getTemplate().getIcon() + " width=32 height=32> <img src=\"l2ui.squaregray\" width=\"32\" height=\"1\"></td>"));
			sb.append("</tr>");
			sb.append("</table>");
		...
		if(command.startsWith("_bbsechant;enchlistpageAtr;"))
		{
			...
			ItemInstance EhchantItem = activeChar.getInventory().getItemByObjectId(ItemForEchantObjID);
			StringBuilder sb = new StringBuilder();
			sb.append(new StringBuilder("Selection: <font color=\"LEVEL\">" + ElementName + "</font><br1> Sharpening selected item:<br1><table width=300>"));
			sb.append(new StringBuilder("<tr><td width=32><img src=icon." + EhchantItem.getTemplate().getIcon() + " width=32 height=32> <img src=\"l2ui.squaregray\" width=\"32\" height=\"1\"></td><td width=236><center>"));
			sb.append(new StringBuilder("<font color=\"LEVEL\">" + EhchantItem.getTemplate().getName() + " " + (EhchantItem.getEnchantLevel() <= 0 ? "" : new StringBuilder("</font><br1><font color=3293F3>Refined to: +" + EhchantItem.getEnchantLevel())) + "</font><br1>"));
			...
			else
			{
				...
				sb.append("<br1>");
				sb.append("<br1>");
				sb.append("<center><font color=\"LEVEL\">Enchant of this thing is not possible!</font></center>");
				sb.append("<br1>");
				sb.append("<br1>");
				sb.append("<br1>");
				sb.append("<br1>");
		...
		if(command.startsWith("_bbsechant;enchantgo;"))
		{
			...
			{
				activeChar.getInventory().destroyItem(pay, EchantPrice, "EnchantCommunity");
				...
				activeChar.sendMessage(new StringBuilder("" + EhchantItem.getTemplate().getName() + " was enchanted to " + EchantVal + ". Thanks.").toString());
		...
		if(command.startsWith("_bbsechant;enchantgoAtr;"))
		{
			...
			if(EhchantItem.isWeapon())
			{
				if(pay != null && pay.getCount() >= EchantPrice)
				{
					activeChar.getInventory().destroyItem(pay, EchantPrice, "Enchant Community");
					if(EhchantItem.isEquipped())
						activeChar.getInventory().unEquipItemInBodySlot(EhchantItem.getEquipSlot());
			...
			else if(EhchantItem.isArmor())
			{
				...
				{
					activeChar.sendMessage("Unable to insert an attribute in armor, not the conditions");
					onBypassCommand(activeChar, "_bbsechant");
					return;
				}
				if(pay != null && pay.getCount() >= EchantPrice)
