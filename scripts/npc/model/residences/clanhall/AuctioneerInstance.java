...
public class AuctioneerInstance extends NpcInstance
{
	 ...
	 * 
	 */
	private static final long serialVersionUID = 3988510064099018835L;
	...
	public void onBypassFeedback(Player player, String command)
	{
		...
		else if (actualCommand.equalsIgnoreCase("list_all"))
		{
			...
			if (events.size() > max)
			{
				...
				msg.replace("%next_bypass%", "npc_%objectId%_list_all " + (page + 1));
			}
			else
			...
			if (page != 0)
			{
				...
				msg.replace("%prev_bypass%", "npc_%objectId%_list_all " + (page - 1));
			}
			else
